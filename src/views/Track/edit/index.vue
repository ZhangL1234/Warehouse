    ...
    <el-form v-model="track" ref="form">
      <div class="form">
        ...
        <div class="btnstyle">
          ...
          <el-button type="danger" class="button" size="small" @click="edit">确认修改</el-button>
        </div>
      </div>
    </el-form>
  </div>
  ...
  methods: {
    ...
    getinfo() {
      ...
      this.$axios({
        ...
        .then(res => {
          // console.log("信息", res.data.list);
          this.id = res.data.list.content[0].id;
          this.track.user_id =
            res.data.list.content[0].user.id &&
            res.data.list.content[0].user.name;
          this.track.phone = res.data.list.content[0].phone;
          this.useraccount = res.data.list.content[0].user.account;
          this.userdate = res.data.list.content[0].user.createDate;
          this.track.workingPlace = res.data.list.content[0].workingPlace;
          this.track.position = res.data.list.content[0].position;
          this.track.jobContent = res.data.list.content[0].jobContent;
          this.track.advice=res.data.list.content[0].advice;
        })
        .catch(() => {});
    },
    //请求选中人员信息
    ...
    //修改人员
    edit() {
      // console.log("用户", this.track.user_id);
      // console.log("本次", this.id);
      this.$axios({
        url: "tracking/edit",
        method: "POST",
        data: {
